...
除该 JSON 数组外，严禁输出任何解释、前缀或代码块标记。`;
  ...
  const DEFAULT_SETTINGS = {
    ...
    semantic: {
      threshold: 0,        // 语义距离阈值，0=关（关的同时下面的近重复也不跑）；越大挡越多
      minSamples: 5,       // 语义层（embedding）「屏蔽」样例少于此数则禁用；近重复层只需 ≥1 条
      minConfidence: 0.25, // 置信度下限：评论的屏蔽相似度须比本批正常基线高出此值才屏蔽（抗小样本误伤）
      dupThreshold: 0.5,   // 第1层 文本近重复（复读查重）阈值：实测正常≤0.11、复读≥0.75，零模型零误伤
      knn: 1,              // 语义层取第 k 近样例：1=最近（提召回），2=更稳（提精确）
    },
    version: 1,
  };
